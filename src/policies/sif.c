...
static int is_better_sjf_job(const job_t *candidate, const job_t *best) {
    if (candidate->estimated_runtime != best->estimated_runtime) {
        return candidate->estimated_runtime < best->estimated_runtime;
    }
...
int policy_select_sjf(const job_queue_t *queue) {
    ...
    for (int i = 1; i < queue->size; i++) {
        if (is_better_sjf_job(queue->items[i], queue->items[best])) {
            best = i;
        }
    }
